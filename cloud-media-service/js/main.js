    ...
    function singleLetters($words) {
        $words.each(function() {
            ...
            for (i in letters) {
                ...
                letters[i] = (selected) ? '</i><i class="in">' + letters[i] + '</i>' : '<i>' + letters[i] + '</i>'
            }
        ...
        if ($word.parents('.cd-headline').hasClass('type')) {
            ...
            $word.parents('.cd-words-wrapper').animate({ 'width': $word.width() + 10 }, revealDuration, function() {
                if ($word.is(':last-child')) {
                    revealAnimationDelayLast = revealAnimationDelay * 1.5;
                    setTimeout(function() { hideWord($word) }, revealAnimationDelayLast)
                } else { setTimeout(function() { hideWord($word) }, revealAnimationDelay) }
            })
        }
    }
